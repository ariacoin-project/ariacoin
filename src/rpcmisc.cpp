...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total ariacoin balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized ariacoin balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in ariacoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in ariacoin/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"ariacoinaddress\"\n"
            "\nReturn information about the given ariacoin address.\n"
            "\nArguments:\n"
            "1. \"ariacoinaddress\"     (string, required) The ariacoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"ariacoinaddress\", (string) The ariacoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: ariacoin address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are ariacoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) ariacoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"ariacoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"ariacoinaddress\"  (string, required) The ariacoin address to use for the signature.\n"
