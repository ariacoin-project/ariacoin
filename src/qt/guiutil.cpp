...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ariacoin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no ariacoin: URI
...
{
    // Convert ariacoin:// to ariacoin:
    //
    //    Cannot handle this later, because ariacoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ariacoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ariacoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ariacoin.lnk
...
{
    return GetAutostartDir() / "ariacoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ariacoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ariacoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ariacoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ariacoin app to startup item list
