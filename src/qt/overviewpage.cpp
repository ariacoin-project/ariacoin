...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeariacoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeariacoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeariacoinAmount = strAnonymizeariacoinAmount.remove(strAnonymizeariacoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeariacoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeariacoinAmount * COIN) nMaxToAnonymize = nAnonymizeariacoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeariacoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeariacoinAmount));
        strAnonymizeariacoinAmount = strAnonymizeariacoinAmount.remove(strAnonymizeariacoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeariacoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
